...
**The squad specialist names referenced in this crusade (e.g. `go-concurrency-purist`) are no longer registered Claude Code subagents.** Their definitions live on disk at `specialists/go/<name>.md` and are loaded ONLY when a crusade runs.
...
Wherever this crusade says "spawn `go-concurrency-purist`", "uses `go-concurrency-purist` agent", "Task tool: subagent_type: `go-concurrency-purist`", or "Use the `go-concurrency-purist` agent", it means: **load `specialists/go/go-concurrency-purist.md` via the protocol above and dispatch via `general-purpose`.** The squad mission text and assigned files are unchanged — only the dispatch mechanism has moved from registered subagent to inline body.
