...
**The squad specialist names referenced in this crusade (e.g. `arch-circular-purist`) are no longer registered Claude Code subagents.** Their definitions live on disk at `specialists/arch/<name>.md` and are loaded ONLY when a crusade runs.
...
Wherever this crusade says "spawn `arch-circular-purist`", "uses `arch-circular-purist` agent", "Task tool: subagent_type: `arch-circular-purist`", or "Use the `arch-circular-purist` agent", it means: **load `specialists/arch/arch-circular-purist.md` via the protocol above and dispatch via `general-purpose`.** The squad mission text and assigned files are unchanged — only the dispatch mechanism has moved from registered subagent to inline body.
