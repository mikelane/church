...
**The squad specialist names referenced in this crusade (e.g. `dep-bloat-purist`) are no longer registered Claude Code subagents.** Their definitions live on disk at `specialists/dep/<name>.md` and are loaded ONLY when a crusade runs.
...
Wherever this crusade says "spawn `dep-bloat-purist`", "uses `dep-bloat-purist` agent", "Task tool: subagent_type: `dep-bloat-purist`", or "Use the `dep-bloat-purist` agent", it means: **load `specialists/dep/dep-bloat-purist.md` via the protocol above and dispatch via `general-purpose`.** The squad mission text and assigned files are unchanged — only the dispatch mechanism has moved from registered subagent to inline body.
