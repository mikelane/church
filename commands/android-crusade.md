...
**The squad specialist names referenced in this crusade (e.g. `android-background-purist`) are no longer registered Claude Code subagents.** Their definitions live on disk at `specialists/android/<name>.md` and are loaded ONLY when a crusade runs.
...
Wherever this crusade says "spawn `android-background-purist`", "uses `android-background-purist` agent", "Task tool: subagent_type: `android-background-purist`", or "Use the `android-background-purist` agent", it means: **load `specialists/android/android-background-purist.md` via the protocol above and dispatch via `general-purpose`.** The squad mission text and assigned files are unchanged — only the dispatch mechanism has moved from registered subagent to inline body.
