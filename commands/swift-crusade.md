...
**The squad specialist names referenced in this crusade (e.g. `swift-api-purist`) are no longer registered Claude Code subagents.** Their definitions live on disk at `specialists/swift/<name>.md` and are loaded ONLY when a crusade runs.
...
Wherever this crusade says "spawn `swift-api-purist`", "uses `swift-api-purist` agent", "Task tool: subagent_type: `swift-api-purist`", or "Use the `swift-api-purist` agent", it means: **load `specialists/swift/swift-api-purist.md` via the protocol above and dispatch via `general-purpose`.** The squad mission text and assigned files are unchanged — only the dispatch mechanism has moved from registered subagent to inline body.
