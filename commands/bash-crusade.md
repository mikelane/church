...
**The squad specialist names referenced in this crusade (e.g. `bash-injection-purist`) are no longer registered Claude Code subagents.** Their definitions live on disk at `specialists/bash/<name>.md` and are loaded ONLY when a crusade runs.
...
Wherever this crusade says "spawn `bash-injection-purist`", "uses `bash-injection-purist` agent", "Task tool: subagent_type: `bash-injection-purist`", or "Use the `bash-injection-purist` agent", it means: **load `specialists/bash/bash-injection-purist.md` via the protocol above and dispatch via `general-purpose`.** The squad mission text and assigned files are unchanged — only the dispatch mechanism has moved from registered subagent to inline body.
