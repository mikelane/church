...
**The squad specialist names referenced in this crusade (e.g. `naming-file-purist`) are no longer registered Claude Code subagents.** Their definitions live on disk at `specialists/naming/<name>.md` and are loaded ONLY when a crusade runs.
...
Wherever this crusade says "spawn `naming-file-purist`", "uses `naming-file-purist` agent", "Task tool: subagent_type: `naming-file-purist`", or "Use the `naming-file-purist` agent", it means: **load `specialists/naming/naming-file-purist.md` via the protocol above and dispatch via `general-purpose`.** The squad mission text and assigned files are unchanged — only the dispatch mechanism has moved from registered subagent to inline body.
